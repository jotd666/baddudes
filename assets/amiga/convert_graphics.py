...
def load_contexted_tileset(tile_sheet_dict,context,nb_colors,is_bob,
                        postload_callback=None,forced_palette=set(),reused_colors=None,ignore_first_color=False):
...
##        tile_palette.add((0,0,0))
    ...
    if lfp>nb_colors:
        ...
        while True:
            ...
            if len(qc) < nb_colors:
                nb_colors_to_try += 1
                # store the last valid quantization but only if it is within bounds
                # (without the reused colors!)
                if (len(set(quantized.values()))) <= nb_colors:
        ...
        if len(qc) > nb_colors:
            ...
            qc = set(quantized.values())
            print(f"quantize overshoot {new_qc} > {nb_colors}, backtracking to {len(qc)}")

        lfp = len(qc)       # update nb colors
        if len(qc) > nb_colors:
            # should not happen but just in case, else data file is corrupt and it takes time to investigate!
            raise Exception("Still too many colors after quantize passes, something went very wrong!!!")
...
def process_tile_context(context_name,tile_sheet_dict,nb_colors,is_bob=False,shift_palette_count=0,first_pass=False,
                            first_colors=None,postload_callback=None,forced_palette=set(),reuse_colors_from=None,ignore_first_color=False):
    ...
    plus "ignore_first_color" was recently added, as if color 0 (black) is used in sprites in some levels with sprite layer,
    the sprite will show through it.
    """
    ...
    postload_callback=postload_callback,forced_palette=forced_palette,reused_colors=reused_colors,ignore_first_color=ignore_first_color)
    tile_24a000_cache = {}
    ...
    if shift_palette_count:
        ...
        if ignore_first_color:
            raise Exception("unsupported ignore_first_color + first_colors combination")
...
##            bg_palette = new_bg_palette
    ...
    # increase number of colors now that quantize has been applied


    nb_total_colors = nb_colors+shift_palette_count
    if len(bg_palette)<nb_total_colors:
        # pad with impossible colors, head first
        bg_palette = [impossible_color]*(nb_total_colors-len(bg_palette)) + bg_palette
    ...
    if first_pass:
        ...
        if ignore_first_color:
            # change color 0 (background) to black again
            bg_palette[0] = (0,0,0)
...
with open(this_dir / "macro_sprite_sizes.json") as f:
    macro_sprite_sizes = json.load(f)

with open(src_dir / "object_dimension_table.68k","w") as f:
    f.write(f"* this file was generated by {this_file}\n")
    f.write(f"NB_MACRO_OBJECTS = {len(macro_sprite_sizes)}\n")
    f.write("object_dimension_table:\n")
    for i,(w,h) in enumerate(macro_sprite_sizes):
        f.write(f"\t.word\t{w//8},{h}\t| {i}\n")
...
if generate_for_levels[4]:
    ...
    reuse_colors_from="level_4_24a000",ignore_first_color=True)
...
if generate_for_levels[6]:
    ...
    process_tile_context("game_level_6",sprite_sheet_dict,48,is_bob=True,shift_palette_count=16,ignore_first_color=True)
